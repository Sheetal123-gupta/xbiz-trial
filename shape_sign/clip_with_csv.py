...
# === will create empty list ===
class_labels = []
text_features = []

#loop iterate hoga jisme 
...
# === TRACK COUNTERS PER LABEL ===
label_counters = defaultdict(int)

# === CSV LOG FUNCTION ===
def log_result(writer, filename, label, confidence, status="success", remark=""):
    formatted_conf = f"{confidence:.2f}" if confidence is not None else ""
    writer.writerow([filename, label, formatted_conf, status, remark])
...
with open(csv_path, "w", newline="", encoding="utf-8") as f:
    ...
    for root, _, files in os.walk(image_folder):
        for filename in sorted(files):  # preserve order
            ...
            try:
                ...
                log_result(writer, filename, "Unreadable", None, "failed", str(e))
                continue
            ...
            try:
                ...
                # === FILENAME LOGIC ===
                prefix = label_prefix.get(label_to_draw, "unknown")
                label_counters[prefix] += 1
                new_filename = f"{prefix}{label_counters[prefix]}.jpg"

                save_path = os.path.join(output_folder, new_filename)
                image.save(save_path)

                # === LOG TO CSV ===
                log_result(writer, new_filename, label_to_draw, avg_conf)

            except Exception as e:
                log_result(writer, filename, "Error", None, "failed", str(e))
